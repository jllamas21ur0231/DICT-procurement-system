...
export default function SAdminDashboard() {
    ...
    useEffect(() => {
        ...
        const fetchAll = async () => {
            try {
                const procRes = await fetch("/sadmin/procurements", {
                    headers: { Accept: "application/json" },
                    credentials: "include",
                });
                if (!procRes.ok) throw new Error("Failed to load procurements.");
                const procData = await procRes.json();
                const list = Array.isArray(procData) ? procData : Array.isArray(procData?.data) ? procData.data : [];
                if (!mounted) return;
                setAll(list);

                // Users count — best-effort
                try {
                    const usersRes = await fetch("/sadmin/users", {
                        headers: { Accept: "application/json" },
                        credentials: "include",
                    });
                    if (usersRes.ok) {
    ...
    // ── Derived: unique modes & projects ─────────────────────────────────────────
    const modeOptions = useMemo(() => {
        const names = [...new Set(all.map((p) => p.mode_of_procurement || p.procurementMode?.name).filter(Boolean))];
        return names.sort();
    }, [all]);

    const projectOptions = useMemo(() => {
        const names = [...new Set(all.map((p) => p.project || p.projectRecord?.name).filter(Boolean))];
        return names.sort();
    }, [all]);
    ...
    const statCards = [
        ...
        ...(userCount !== null ? [{ number: userCount, label: "Total", sublabel: "Users", color: "text-[#134C62]" }] : []),
    ];
    ...
    return (
        <div>
            ...
            {/* Stats Cards */}
            <div className={`grid grid-cols-${statCards.length} gap-6 mb-8`}>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading && (
                                <tr><td colSpan={7} className="px-6 py-10 text-center text-gray-400">Loading procurements…</td></tr>
                            )}
                            {!loading && error && (
                ...
                <div className="flex items-center justify-between px-6 py-3 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => setPage((p) => p - 1)} disabled={page === 1} className="px-2 py-1 rounded text-sm disabled:opacity-40 hover:bg-gray-200 transition-colors">‹</button>
                        <span className="px-3 py-1 text-sm font-medium">{page} / {totalPages}</span>
                        <button onClick={() => setPage((p) => p + 1)} disabled={page === totalPages} className="px-2 py-1 rounded text-sm disabled:opacity-40 hover:bg-gray-200 transition-colors">›</button>
